...
src_compile() {
	...
	# Strip -mllvm flags from CGO environment, Go toolchain does not support them
	local _mllvm_re='s/-mllvm[[:space:]]+[^[:space:]]+//g'
	CGO_CFLAGS=$(sed -E "${_mllvm_re}" <<< "${CGO_CFLAGS}")
	CGO_CXXFLAGS=$(sed -E "${_mllvm_re}" <<< "${CGO_CXXFLAGS}")
	CGO_LDFLAGS=$(sed -E "${_mllvm_re}" <<< "${CGO_LDFLAGS}")
